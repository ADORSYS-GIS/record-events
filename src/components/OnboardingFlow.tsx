...
import { Shield, Camera, Key, CheckCircle, ArrowRight, Sparkles, Sun, Moon, Monitor, Smartphone, Lock, Zap } from "lucide-react";
...
const OnboardingFlow: React.FC<OnboardingFlowProps> = ({ onComplete, i18n, keyStatus, isKeyGenerating }) => {
  ...
  useEffect(() => {
    if (currentStep === 3 && !isKeyGenerating && keyStatus && keyStatus.includes('successfully')) {
      setSecurityStepCompleted(true);
      setTimeout(() => {
        completeStep(4); // Complete the key generation step (new ID 4)
      }, 1500);
    } else if (currentStep === 4 && securityStepCompleted) { // After key generation is completed
      setTimeout(() => {
  ...
  const steps: OnboardingStep[] = [
    {
      ...
      description: t("onboarding.welcome.description", "Your secure platform for reporting events with blockchain verification"),
      icon: <Sparkles className="w-20 h-20 text-primary-500" />,
      type: 'welcome',
      status: currentStep === 0 ? 'active' : completedSteps.includes(1) ? 'completed' : 'pending'
    },
    {
      id: 2,
      title: t("onboarding.features.secure.title", "Secure & Private"),
      description: t("onboarding.features.secure.description", "Your data is encrypted and verified on the blockchain for maximum security"),
      icon: <Lock className="w-20 h-20 text-primary-500" />,
      type: 'feature',
      status: currentStep === 1 ? 'active' : completedSteps.includes(2) ? 'completed' : 'pending'
    },
    {
      id: 3,
      title: t("onboarding.permissions.title", "Camera Access"),
      description: t("onboarding.permissions.description", "Allow camera access to capture event photos"),
      ...
      status: currentStep === 2 ? 'active' : completedSteps.includes(3) ? 'completed' : 'pending'
    },
    {
      id: 4,
      title: t("onboarding.security.title", "Generating Secure Keys"),
      description: t("onboarding.security.description", "Creating your unique cryptographic key pair for secure event reporting. This may take a moment."),
      icon: <Key className="w-20 h-20 text-primary-500" />,
      type: 'security',
      status: currentStep === 3 ? 'active' : completedSteps.includes(4) ? 'completed' : 'pending'
    },
    {
      id: 5,
      title: t("onboarding.ready.title", "You're Ready!"),
      description: t("onboarding.ready.description", "Start reporting events securely"),
      icon: <CheckCircle className="w-20 h-20 text-success-500" />,
      type: 'ready',
      status: currentStep === 4 ? 'active' : completedSteps.includes(5) ? 'completed' : 'pending'
    }
  ];

  const completeStep = useCallback((stepId: number) => {
    setCompletedSteps(prev => [...prev, stepId]);
    if (stepId === 5) { // This is the final "You're Ready!" step
        ...
        <div className="mb-12">
              ...
              <div key={step.id} className="flex flex-col items-center">
                ...
                `}>
                  {step.status === 'completed' ? (
                    ...
                    <span className="text-sm sm:text-base font-semibold">{step.id}</span>
                  )}
                </div>
          ...
          <div className="w-full bg-neutral-200 dark:bg-neutral-700 rounded-full h-3 overflow-hidden">
            ...
            />
                  ...
                  {/* Step Content */}
          <div className={`onboarding-content rounded-3xl shadow-xl p-10 sm:p-16 animate-fade-in transition-all duration-700 ease-in-out transform ${
            ...
            <div className="text-center mb-12">
              ...
              <p className="text-neutral-600 dark:text-neutral-300 leading-relaxed text-lg sm:text-xl max-w-lg mx-auto animate-slide-up" style={{animationDelay: '0.2s'}}>
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-primary-700 dark:text-primary-300">
                      {t("onboarding.features.securityDesc", "Your data is encrypted and verified on the blockchain")}
                    </p>
                  </div>
                </div>
              </div>
